; (function ($) {
    // 添加原型方法   $('xxx').paging({ nowPage： 1，  pageNum： xx， callback：fn})
    $.fn.pagenation = function (options) {
        'use strict'; // 启动严格模式
        var arr = []  // 定义一个数组

        // 构造函数
        function Paging(element, options) { // dom元素  配置
            this.element = element; // 挂载dom元素
            this.options = { // 挂载opitons
        ...
        // 构造函数的原型
        Paging.prototype = {
            constructor: Paging, // 还原构造器
            init: function () {
                this.createHtml(); // 创建html节点
                this.bindClickEvent(); // 绑定事件
                this.disabled(); // 是否禁用
            },
            // 创建html
            createHtml: function () {
